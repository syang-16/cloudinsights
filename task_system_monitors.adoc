...
|ONTAP Volume Low Snapshot Count|CRITICAL @ < 10 |When a volume has 10 or fewer snapshots, it minimizes the ability to recover data in the case of data loss or corruption. To correct this, try increasing the number of snapshots in the snapshot policy to ensure sufficient local data protection.| Consider using the default Snapshot policy. The default Snapshot policy creates hourly, daily, and weekly Snapshot copies, retaining six hourly, two daily, and two weekly Snapshot copies.
If there is not enough space for the default snapshot policy, then try increasing the volume size to provide sufficient space.
For customers who need more protection than the default snapshot policy, create a new Snapshot policy with schedules tailored to this volume’s data and pattern of operations.
|ONTAP Volume Snapshot Reserve is Underutilized|WARNING @ > 355 days|A volume's snapshot reserve is underutilized when less than 50% of snapshot reserve is projected to be utilized in the next 6 months. To correct this, reallocate some of the volume's underutilized snapshot reserve capacity for data storage.|Some of the volume's underutilized snapshot reserve capacity can be reallocated to data storage for use by the client application. This can be done by configuring the reserve percentage to a lower value.
For instance, reducing the reserve capacity from 5% to 4% of total capacity for a 100 TiB volume, would free up 1 TiB more data storage by client application.
To modify the snapshot copy reserve, run the ONTAP command:

volume modify -vserver <vserver name> -volume <volume name> -percent-snapshot-space <snap reserve>
