...
|ONTAP Volume Snapshot Reserve is Underutilized|WARNING|A volume's snapshot reserve is underutilized when less than 50% of snapshot reserve is projected to be utilized in the next 6 months. To correct this, reallocate some of the volume's underutilized snapshot reserve capacity for data storage.|Some of the volume's underutilized snapshot reserve capacity can be reallocated to data storage for use by the client application. This can be done by configuring the reserve percentage to a lower value.…For instance, reducing the reserve capacity from 5% to 4% of total capacity for a 100 TiB volume, would free up 1 TiB more data storage by client application.…To modify the snapshot copy reserve, run the ONTAP command:…volume modify -vserver <vserver name> -volume <volume name> -percent-snapshot-space <snap reserve>
