...
|ONTAP Volume Snapshot Reserve is Underutilized|WARNING|A volume's snapshot reserve is underutilized when less than 50% of snapshot reserve is projected to be utilized in the next 6 months. To correct this, reallocate some of the volume's underutilized snapshot reserve capacity for data storage.|Some of the volume's underutilized snapshot reserve capacity can be reallocated to data storage for use by the client application. This can be done by configuring the reserve percentage to a lower value.
