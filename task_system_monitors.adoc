...
|ONTAP Volume Low Snapshot Count|•|Filter(s):…"creation_time" >= 14 days…"is_protected_by_snapshot_policy" == True…"snapshot_capacity_time_to_full" >= 90 days…Condition(s):…"snapshot_count" <= 10 days|When a volume has 10 or fewer snapshots, it minimizes the ability to recover data in the case of data loss or corruption. To correct this, try increasing the number of snapshots in the snapshot policy to ensure sufficient local data protection.|1.|Performing the following corrective actions:…Consider using the default Snapshot policy. The default Snapshot policy creates hourly, daily, and weekly Snapshot copies, retaining six hourly, two daily, and two weekly Snapshot copies.…If there is not enough space for the default snapshot policy, then try increasing the volume size to provide sufficient space.…For customers who need more protection than the default snapshot policy, create a new Snapshot policy with schedules tailored to this volume’s data and pattern of operations.|2.
|ONTAP Volume Snapshot Reserve is Underutilized|•|Filters:…'space_guarantee' = volume…'size_used_percent' >= 75% …'volume_age' >= 90 days …'contains_luns' = No…'is_vserver_root' = No…'snapshot_reserve_percent' > 1%…'size_total' > 100GiB…'snapshot_size_used_percent' <= 45%…Condition:…snapshot_capacity_time_to_full_total > 355|A volume's snapshot reserve is underutilized when less than 50% of snapshot reserve is projected to be utilized in the next 6 months. To correct this, reallocate some of the volume's underutilized snapshot reserve capacity for data storage.|Performing the following corrective actions:…Some of the volume's underutilized snapshot reserve capacity can be reallocated to data storage for use by the client application. This can be done by configuring the reserve percentage to a lower value.…For instance, reducing the reserve capacity from 5% to 4% of total capacity for a 100 TiB volume, would free up 1 TiB more data storage by client application.…To modify the snapshot copy reserve, run the ONTAP command:…volume modify -vserver <vserver name> -volume <volume name> -percent-snapshot-space <snap reserve>
...
=== Synthetic Metric Monitors

|===
|Monitor Name|Severity|Monitor Description|Corrective Action|
|Storage Capacity Time To Full|CRITICAL|When a storage pool (aggregate) is filling up, I/O operations slow down and eventually stop, resulting in a storage outage incident. To prevent such storage outage, this monitor tracks storage capacity's Time To Full (TTF), a metric that predicts when the storage pool is likely to run out of capacity. Once this metric crosses a critical threshold, the monitor preemptively alerts customers with the predicted number of days until the storage pool exhausts its capacity. This preemptive alert allows customers enough time to take proactive actions to ensure sufficient storage capacity and avoid a potential storage outage.…A warning alert indicates that a planned action should be taken soon to restore minimum free space.…A critical alert indicates that service disruption is imminent and emergency measures should be taken to free up space to ensure service continuity.|If the critical threshold is breached, immediately consider the following actions to minimize service disruption:
1. Delete Snapshots on non-critical volumes.
2. Delete Volumes or LUNs that are non-essential workloads and that may be restored from off-storage copies.……If the warning threshold is breached, plan the following immediate actions:
1. Move one or more volumes to a different storage location.
2. Add more storage capacity.
3. Change storage efficiency settings or tier inactive data to cloud storage.|
|ONTAP Volume Capacity Time To Full|CRITICAL|When a volume is filling up, I/O operations slow down and eventually stop, resulting in a capacity outage incident. To prevent such capacity outage, this monitor tracks volume capacity's Time To Full (TTF), a metric that predicts when the volume is likely to run out of capacity. Once this metric crosses a critical threshold, the monitor preemptively alerts customers the predicted number of days until the volume exhausts its capacity. This preemptive alert allows customers enough time to take proactive actions to ensure sufficient volume capacity and avoid a potential capacity outage.…A warning alert indicates that a planned action should be taken soon to restore minimum based free space.…A critical alert indicates that service disruption is imminent and emergency measures should be taken to free up space to ensure service continuity.…|If critical threshold is breached, consider following immediate actions to minimize service disruption:
1. Increase the space of the volume to accommodate growth.
2. Delete unwanted data to free up space.
3. If snapshot copies occupy more space than the snapshot reserve, delete old Snapshots or enable Volume Snapshot Autodelete.…If warning threshold is breached, plan to take the following immediate actions:
1. Increase the space of the volume in order to accommodate growth
2. If snapshot copies occupy more space than the snapshot reserve, delete old Snapshots or enabling Volume Snapshot Auto-delete.|
|ONTAP Volume Snapshot Capacity Time To Full|CRITICAL|Snapshot reserved space in an ONTAP volume is a portion of its storage capacity that is used to store snapshots, which allow data to be protected locally. The more new and updated data is stored in the volume, the more snapshot capacity is used, and less snapshot capacity is available for the data. If the Snapshot copies exceed the reserve space, they spill into the active file system, thus exhausting the volume's storage capacity and potentially causing a storage outage.…To prevent such data storage outage, this monitor tracks snapshot capacity's Time To Full (TTF), a metric that predicts when a volume is likely to run out of reserve space for snapshots. Once this metric crosses a critical threshold, the monitor preemptively alerts customers the predicted number of days until the volume exhausts its snapshot capacity. This preemptive alert allows customers enough time to take proactive actions to ensure sufficient data protection in the volume.|If the critical threshold is breached, consider immediate actions to minimize service disruption:
1. Increase the snapshot reserve space within the volume to accommodate the growth.
2. Delete some older, unwanted snapshots to free up space.…
If the warning threshold is breached, plan to take the following immediate actions:
1. Increase the snapshot reserve space within the volume to accommodate the growth. 
2. Configure snapshots to use data space in the volume when the snapshot reserve is full.
