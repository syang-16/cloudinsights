...
On an asset landing page, tables showing infrastructure objects (storage, volume, VM, etc.) can be exported to a .CSV file.  

On the desired table you wish to export, click the "three dots" menu and select _Export to CSV_.
...
Note that the table must be displaying data in order for the "export" option to be shown, and the table must be grouped. You can choose a grouping by opening the table in Edit mode and selcting the desired "group by" attribute.
